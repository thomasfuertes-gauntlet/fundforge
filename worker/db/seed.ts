...
for (const c of campaigns) {
  lines.push(`INSERT INTO campaigns (id, organizer_id, title, category, status, hero_image, images, goal, stretch_goal, matching_sponsor, raised, backer_count, average_gift, created_at, ends_at, ended_at, days_left, weekly_momentum, story, updates, testimonials, summary) VALUES (${esc(c.id)}, ${esc(c.organizerId)}, ${esc(c.title)}, ${esc(c.category)}, ${esc(c.status)}, ${esc(c.heroImage)}, ${esc(c.images || [])}, ${c.goal}, ${esc(c.stretchGoal || null)}, ${esc(c.matchingSponsor || null)}, ${c.raised}, ${c.backerCount}, ${c.averageGift}, ${esc(c.createdAt)}, ${esc(c.endsAt || null)}, ${esc(c.endedAt || null)}, ${c.daysLeft ?? "NULL"}, ${c.weeklyMomentum ?? 0}, ${esc(c.story || [])}, ${esc(c.updates || [])}, ${esc(c.testimonials || [])}, ${esc(c.summary || null)});`);
}
...
for (const d of donations) {
  lines.push(`INSERT INTO donations (id, campaign_id, donor_name, amount, timestamp, message) VALUES (${esc(d.id)}, ${esc(d.campaignId)}, ${esc(d.donorName)}, ${d.amount}, ${esc(d.timestamp)}, ${esc(d.message)});`);
}
